...
| status | string | 状态(active=正常/frozen=冻结) |
| has_password | bool | 是否已设置支付密码 |
| total_recharged | decimal | 累计充值金额 |
| total_consumed | decimal | 累计消费金额 |
...
| id | int | 交易ID |
| type | string | 交易类型 |
| type_text | string | 类型中文描述 |
| amount | decimal | 交易金额 |
| direction | string | 方向(in=收入/out=支出) |
| balance_after | decimal | 交易后余额 |
| remark | string | 备注 |
| created_at | string | 交易时间(ISO8601) |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| amount | decimal | 是 | 充值金额(最低1元，最高由后台配置，默认200元) |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| password | string | 是 | 6位数字密码(正则: `^\d{6}$`) |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| old_password | string | 是 | 原6位数字密码 |
| new_password | string | 是 | 新6位数字密码(正则: `^\d{6}$`) |
...
```
POST /v2/wallet/password/verify
```
...
> 密码错误超过5次（可配置）将锁定30分钟（可配置）

---

### 10.7 钱包支付（混合支付）

钱包支付集成在订单支付接口中，使用以下参数：
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| use_wallet | bool | 否 | 是否使用钱包余额(默认false) |
| payment_password | string | 条件必填 | 支付密码(use_wallet=true时必填) |
