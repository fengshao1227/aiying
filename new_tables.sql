...
CREATE TABLE `customers` (
  ...
  `package_name` varchar(100) DEFAULT NULL COMMENT '套餐名称（春/夏/秋）',
...
CREATE TABLE `rooms` (
  ...
  `room_name` varchar(50) NOT NULL COMMENT '房间名称（24节气命名）',
  `floor` tinyint NOT NULL COMMENT '楼层：1=一楼，2=二楼',
  `room_type` varchar(50) DEFAULT NULL COMMENT '房型（标准间/套房/双床房等）',
  `color_code` varchar(7) NOT NULL DEFAULT '#FFFFFF' COMMENT '颜色代码（用于房态展示）',
...
VALUES ('admin', '$2y$12$LQv3c1yycJidQMTjBW3jJ.k5z8qLWJEcHDrZQHqJqRxMxl3dTkEwq', '系统管理员', 1, NOW(), NOW());

-- ============================================================
-- 说明：
-- 1. 执行前请确保已备份数据库
-- 2. customers 表通过 phone 字段与 users 表关联
-- 3. 外键约束确保数据完整性
-- 4. 默认管理员密码为 admin123，首次登录后请修改
-- ============================================================
